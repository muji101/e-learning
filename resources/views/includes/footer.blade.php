{{-- <div id="particles-js" class="relative footer w-full h-96 bg-gradient-to-r from-gray-900 to-blue-900">
    <!-- footer -->
    <div class="relative">
        
    </div>
</div> --}}

<div id="footer" class="top-0 grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5  text-gray-100 p-4 md:py-24 md:px-20 bg-gradient-to-t from-gray-700 to-blue-900">
    <div class="col-span-2 text-lg" data-aos="fade-up" data-aos-delay="100">
        <div class="">
            <img src="{{ asset('/images/logo.png') }}" alt="" class="w-16 md:w-40">
        </div>
        <p class="mt-4 mb-8">
            Go Sinau adalah solusi belajar seputar teknologi yang mudah dimengerti dalam pembelajaran 
        </p>
        <p>
            ©Go Sinau 2021. All rights reserved
        </p>
    </div>
    <div class="col-span-1 pt-8 md:ml-10 md:pt-0 " data-aos="fade-up" data-aos-delay="200">
        <h1 class="text-lg font-bold">Media Social</h1>
        <div class="flex flex-col md:space-y-2 mt-2 md:mt-6 text-base">
            <a href="#">Telegram</a>
            <a href="#">Instagram</a>
            <a href="#">Facebook</a>
            <a href="#">Whatsapp</a>
        </div>
    </div>
    <div class="col-span-1 pt-8 md:pt-0" data-aos="fade-up" data-aos-delay="400">
        <h1 class="text-lg font-bold">Mentor</h1>
        <div class="flex flex-col md:space-y-2 mt-2 md:mt-6 text-base">
            <a href="#">Coding</a>
            <a href="#">Design</a>
            <a href="#">Marketing</a>
        </div>
    </div>
    <div class="col-span-1 pt-8 md:pt-0" data-aos="fade-up" data-aos-delay="500">
        <h1 class="text-lg font-bold">Help</h1>
        <div class="flex flex-col md:space-y-2 mt-2 md:mt-6 text-base">
            <a href="#">Help center</a>
            <a href="#">Contact support</a>
            <a href="#">Instructions</a>
            <a href="#">How it works</a>
        </div>
    </div>
</div>